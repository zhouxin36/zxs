...
面试官你好，我叫周鑫，18年毕业于东莞理工学院软件工程卓越计划专业；17年12月入职团贷网，现今3年工作经验，在北明担任Java高级开发工程师；20年通过阿里云云计算架构师ACE认证、Linux基金会与CNC联合的CKAD认证；最近在深入学习Java性能调优相关内容，例如JVM的运行时数据区、类加载器、JIT编译器等，还有jvisualVM和JMC等工具，希望能在贵公司发挥我的能力，谢谢
...
* 布局
  ...
  * instance object：oops普通对象指针压缩开启，超过32G关闭
    * 32G：`2*35`；所以限制在32G以内，地址左移三位后`2*32`；只能访问8的倍数的地址空间；所以对象8倍补齐；如果64G就要16位补齐；
...
* 堆内存优化
  ...
  * 堆分析
    ...
    * 堆转储
      ...
      *  对象大小
         *  浅对象大小：当前对象大小，包括引用指针
         *  保留对象大小：包括当前对象大小，引用的对象大小（共享对象除外）
         *  深对象大小：包括当前对象大小，引用的对象大小
    * OOM原因
      * 原生系统内存不足
        *  比如32位系统，一个进程最大内存4G，缺指定了3.8G的堆
      * 堆或元空间内存不足
        *  本身不足：扩容
        *  内存泄漏：
      * GC达到限制要求
        * 要求
          *  一次Full GC占用时间达到 -XX:GCTimeLimit要求，默认98%
          *  一次Full GC回收内存少于 -XX:GCHeapFreeLimit，默认2%
          *  上面两个条件连续5次成立
          *   -XX:GCHeapFreeLimit=true 开启，默认开启
        *  第五次GC时，作为最后一搏，会释放所有软引用，以防止该错误
  * 减少内存使用
    * 减少对象大小
      *  减少对象数量
      *  减少实例大小：压缩类指针，普通对象指针
    *  懒加载
    *  尽早清理：对于集合来说非常有必要
    *  通过维护不可变对象的标准化对象，减少冗余对象
  * 生命周期管理
    * 对象池
      *  适合创建对象成本非常高（至少高过老年代GC扫描带来的影响），数量少（GC影响小）
    * 线程本地变量
      *  适合绑定线程的重用对象
* 原生内存优化
  *   -Xms512m -Xmx2048m：512M提交（committed）内存，2G（reserved）保留内存（JVM告诉操作系统，操作系统承诺的内存）
  * 内存占用
    *  堆
    *  线程栈
    *  代码缓存
    * 本地库调用
      *  没有工具可以查看
  * 追踪
    *  开启：-XX:NativeMemoryTracking=off|summary|detail
    *   jcmd 19628 VM.native_memory summary
    *  开启基准线（做对比）： jcmd process_id VM.native_memory baseline
    *  对比： jcmd process_id VM.native_memory summary.diff
* 操作系统优化
  * 大页：大的页
    *  页：操作系统分配内存的最小单元
    *  全局页表
    *  TLB：存在高速缓存中的LRU缓存，访问比页表快
* 字符串优化
  * 使用intern() 标准化字符串
    *  intern使用hashtable保存String
  *  在G1垃圾收集器中，-XX:+UseStringDeduplication 查找所有重复字符串并删除、修改引用
  * 字符串连接
    *  Java 8 中使用StringBuilder优化处理字符串连接
    *  Java 11使用JVM的特殊方法优化
